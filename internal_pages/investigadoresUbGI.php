<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <meta name="viewport"
        content="width=device-width, user-scalable=no, initial-scale=1, maxium-scale=1.0, minium-scale=1.0, shrink-to-fit=no">
    <link rel="stylesheet" href="../css/bootstrap.min.css">
    <title>UbGI | INVESTIGADORES</title>
    <link rel="icon" href="../img/iconoub.jpg">
    <link href="https://fonts.googleapis.com/css2?family=Open+Sans:wght@300&family=Ranchers&display=swap"
        rel="stylesheet">
    <link rel="stylesheet" href="../css/fontello.css">
    <link rel="stylesheet" href="../css/estilos.css">
</head>
<body>
    <nav class="navbar navbar-expand-lg navbar-dark bg-dark fixed-top">
        <div class="container">
            <a href="UbGI.html" class="navbar-brand">Inicio</a>
            <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbar"
                aria-controls="navbar" aria-expanded="false" aria-label="Menu Navegacion ">
                <span class="navbar-toggler-icon"></span>
            </button>

            <div class="collapse navbar-collapse" id="navbar">
                <ul class="navbar-nav mr-auto" id="Menu">
                    <li class="nav-item">
                        <a href="datosGeneralesUbGI.html" target="_blank" class="nav-link">Datos Generales</a>
                    </li>
                    <li class="nav-item">
                        <a href="investigadoresUbGI.html" target="_blank" class="nav-link">Investigadores</a>
                    </li>
                    <li class="nav-item">
                        <a href="produccionUbGI.html" target="_blank" class="nav-link">Producción</a>
                    </li>
                    <li class="nav-item">
                        <a href="clasificacionUbGI.html" target="_blank" class="nav-link">Clasificación</a>
                    </li>
                    <li class="nav-item">
                        <a href="inicioSesionUbGI.html" target="_blank" class="nav-link">Salir</a>
                    </li>

                </ul>

            </div>

        </div>

    </nav>
    <br>
    <div>
        <img id="user" src="../img/user.jpg" class="rounded-circle">
        <img id="campana" src="../img/campana.jpg" class="rounded-circle"><hr>
    </div>
    <div class="text-center">
        <h1 class="display-3" >Investigadores</h1>
        <img id="logoI" src="../img/img3.jpeg" alt="Logo UB" class="rounded-circle"><hr>
    </div>

    <div class="text-justify-center">

    <table border="3">
        <tr>
            <th>Nombre</th>
            <th>Edad</th>
            <th>Cantidad de Productos</th>
        </tr>
         <tr>
            <td>Angie</td>
            <td>200</td>
            <td>300</td>
        </tr>
         <tr>
            <td>Jeniffer</td>
            <td>201</td>
            <td>302</td>
        </tr>

        <tr>
            <td>William</td>
            <td>201</td>
            <td>302</td>
        </tr>
        <tr>
            <td>Gissel</td>
            <td>201</td>
            <td>302</td>
        </tr>
    
    </table>

    <p class="center-content"><input id="boton" type="submit" class="btn btn-outline-danger" value="Vincular"></p>
</div>
    
</body>
</html>